...
public class MoreEnemy extends Enemy {
    ...
    public MoreEnemy(TileMap.Pair initial) {
        ...
        for (int i = 0; i < 7; i++) {
            frameLengths[i] = 1.8f;
            frames[i] = Global.all.getSubimage(Global.tileSize * i, Global.tileSize * 19, Global.tileSize, Global.tileSize);
        }
    ...
    public static MoreEnemy newMoreEnemy(int x, int y) {
        return new MoreEnemy(new TileMap.Pair (x, y));
    } 
